    ...
    suspend fun createOrGetChat(otherUserId: String): Result<Chat> {
        return try {
            ...
            val existingChat = db.collection("chats")
                ...
                .find { doc ->
                    @Suppress("UNCHECKED_CAST") // Suppress for this specific cast
                    val participants = doc.get("participants") as? List<String>
                    participants?.contains(otherUserId) == true && participants.size == 2 // Optional: also check size
                }
